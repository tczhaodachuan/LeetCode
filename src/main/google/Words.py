...
def word_ladder(start_word, end_word, word_list):
    ...
    transform_dict = build_graph(start_word, word_list)
    queue = []
    ...
    while len(queue) != 0:
        ...
        for i in range(queue_size):
            ...
            if curr_word == end_word:
                return steps
            for nxt_word in transform_dict[curr_word]:
                queue.append(nxt_word)
        steps += 1
    return 0


def build_graph(start_word, word_list):
    transform_dict = {}
    if start_word not in word_list:
        transform_dict[start_word] = []
        for word in word_list:
            if diff(start_word, word) == 1:
                transform_dict[start_word].append(word)
    for i, word in enumerate(word_list):
        transform_dict[word] = []
        for sec_word in word_list[:i] + word_list[i + 1:]:
            if diff(word, sec_word) == 1:
                transform_dict[word].append(sec_word)
    return transform_dict


def diff(word, sec_word):
    count = 0
    for i in range(len(word)):
        if word[i] != sec_word[i]:
            count += 1
            if count > 1:
                return count
...
def word_ladder_bidirect_bfs(start_word, end_word, word_dict):
    ...
    while len(s1) >0 and len(s2) > 0:
        ...
        for word in s1:
            for i in range(len(word)):
                for j in string.ascii_lowercase[:31]:
                    if j == word[i]:
                        continue
                    transit_word = word[:i] + j + word[i+1:]
                    if transit_word in s2:
                        return steps + 1
                    elif transit_word in word_dict:
...
def word_ladder_II(start_word, end_word, word_list):
    if end_word not in word_list:
        return []
    word_dict = set(word_list)
    if start_word in word_dict:
        word_dict.remove(start_word)
    if end_word in word_dict:
        word_dict.remove(end_word)
